.class public final Lcom/ironsource/cg;
.super Ljava/lang/Object;
.source "SourceFile"

# interfaces
.implements Lcom/ironsource/Sf;


# instance fields
.field private final a:Landroid/os/Handler;

.field private b:Landroid/webkit/WebView;

.field private final c:Landroid/webkit/WebView;

.field private final d:Lorg/json/JSONObject;

.field private e:Z

.field private f:Z


# direct methods
.method public static synthetic $r8$lambda$2l5ykIyDobinTg7BPFm8nehCU0M(Lcom/ironsource/S8;)V
    .locals 0

    invoke-static {p0}, Lcom/ironsource/cg;->a(Lcom/ironsource/S8;)V

    return-void
.end method

.method public static synthetic $r8$lambda$G9Z4AEfKD5a_Lo7zAbksYRKX2gU(Lcom/ironsource/cg;Ljava/lang/String;)V
    .locals 0

    invoke-static {p0, p1}, Lcom/ironsource/cg;->a(Lcom/ironsource/cg;Ljava/lang/String;)V

    return-void
.end method

.method public static synthetic $r8$lambda$UA4e1NgiCtOjszJS4OKKJFg3JLo(Lcom/ironsource/cg;)V
    .locals 0

    invoke-static {p0}, Lcom/ironsource/cg;->e(Lcom/ironsource/cg;)V

    return-void
.end method

.method public constructor <init>(Lcom/ironsource/mediationsdk/testSuite/TestSuiteActivity;Lcom/ironsource/S8;Lorg/json/JSONObject;Ljava/lang/String;)V
    .locals 2

    const-string v0, "activity"

    invoke-static {p1, v0}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullParameter(Ljava/lang/Object;Ljava/lang/String;)V

    const-string v0, "uiLifeCycleListener"

    invoke-static {p2, v0}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullParameter(Ljava/lang/Object;Ljava/lang/String;)V

    const-string v0, "jsonData"

    invoke-static {p3, v0}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullParameter(Ljava/lang/Object;Ljava/lang/String;)V

    .line 1
    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    .line 8
    new-instance v0, Landroid/os/Handler;

    invoke-virtual {p1}, Landroid/content/Context;->getMainLooper()Landroid/os/Looper;

    move-result-object v1

    invoke-direct {v0, v1}, Landroid/os/Handler;-><init>(Landroid/os/Looper;)V

    iput-object v0, p0, Lcom/ironsource/cg;->a:Landroid/os/Handler;

    .line 9
    new-instance v0, Landroid/webkit/WebView;

    invoke-direct {v0, p1}, Landroid/webkit/WebView;-><init>(Landroid/content/Context;)V

    iput-object v0, p0, Lcom/ironsource/cg;->b:Landroid/webkit/WebView;

    .line 10
    new-instance v0, Landroid/webkit/WebView;

    invoke-direct {v0, p1}, Landroid/webkit/WebView;-><init>(Landroid/content/Context;)V

    iput-object v0, p0, Lcom/ironsource/cg;->c:Landroid/webkit/WebView;

    .line 11
    iput-object p3, p0, Lcom/ironsource/cg;->d:Lorg/json/JSONObject;

    .line 16
    invoke-direct {p0, p1, p2, p4}, Lcom/ironsource/cg;->a(Lcom/ironsource/mediationsdk/testSuite/TestSuiteActivity;Lcom/ironsource/S8;Ljava/lang/String;)V

    return-void
.end method

.method public static final synthetic a(Lcom/ironsource/cg;)Landroid/os/Handler;
    .locals 0

    .line 1
    iget-object p0, p0, Lcom/ironsource/cg;->a:Landroid/os/Handler;

    return-object p0
.end method

.method private static final a(Lcom/ironsource/S8;)V
    .locals 1

    const-string v0, "$uiLifeCycleListener"

    invoke-static {p0, v0}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullParameter(Ljava/lang/Object;Ljava/lang/String;)V

    .line 62
    invoke-interface {p0}, Lcom/ironsource/S8;->onUIReady()V

    return-void
.end method

.method private static final a(Lcom/ironsource/cg;Ljava/lang/String;)V
    .locals 2

    const-string v0, "this$0"

    invoke-static {p0, v0}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullParameter(Ljava/lang/Object;Ljava/lang/String;)V

    const-string v0, "$script"

    invoke-static {p1, v0}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullParameter(Ljava/lang/Object;Ljava/lang/String;)V

    .line 71
    iget-object p0, p0, Lcom/ironsource/cg;->c:Landroid/webkit/WebView;

    new-instance v0, Ljava/lang/StringBuilder;

    const-string v1, "javascript:"

    invoke-direct {v0, v1}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    invoke-virtual {v0, p1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object p1

    invoke-virtual {p1}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object p1

    invoke-static {p0, p1}, Lcom/safedk/android/internal/partials/IronSourceNetworkBridge;->webviewLoadUrl(Landroid/webkit/WebView;Ljava/lang/String;)V

    return-void
.end method

.method public static final synthetic a(Lcom/ironsource/cg;Z)V
    .locals 0

    .line 2
    iput-boolean p1, p0, Lcom/ironsource/cg;->e:Z

    return-void
.end method

.method private final a(Lcom/ironsource/mediationsdk/testSuite/TestSuiteActivity;Lcom/ironsource/S8;Ljava/lang/String;)V
    .locals 3

    .line 3
    iget-object v0, p0, Lcom/ironsource/cg;->b:Landroid/webkit/WebView;

    if-eqz v0, :cond_0

    invoke-virtual {v0}, Landroid/webkit/WebView;->getSettings()Landroid/webkit/WebSettings;

    move-result-object v0

    goto :goto_0

    :cond_0
    const/4 v0, 0x0

    :goto_0
    const/4 v1, 0x1

    if-nez v0, :cond_1

    goto :goto_1

    :cond_1
    invoke-virtual {v0, v1}, Landroid/webkit/WebSettings;->setJavaScriptEnabled(Z)V

    .line 4
    :goto_1
    iget-object v0, p0, Lcom/ironsource/cg;->c:Landroid/webkit/WebView;

    invoke-virtual {v0}, Landroid/webkit/WebView;->getSettings()Landroid/webkit/WebSettings;

    move-result-object v0

    invoke-virtual {v0, v1}, Landroid/webkit/WebSettings;->setJavaScriptEnabled(Z)V

    .line 6
    new-instance v0, Lcom/ironsource/Lf;

    iget-object v1, p0, Lcom/ironsource/cg;->a:Landroid/os/Handler;

    invoke-direct {v0, p1, v1}, Lcom/ironsource/Lf;-><init>(Lcom/ironsource/mediationsdk/testSuite/TestSuiteActivity;Landroid/os/Handler;)V

    .line 9
    new-instance p1, Lcom/ironsource/cg$b;

    invoke-direct {p1, p0, p2}, Lcom/ironsource/cg$b;-><init>(Lcom/ironsource/cg;Lcom/ironsource/S8;)V

    .line 22
    iget-object v1, p0, Lcom/ironsource/cg;->c:Landroid/webkit/WebView;

    .line 23
    new-instance v2, Lcom/ironsource/Rf;

    invoke-direct {v2, v0, p1, p0}, Lcom/ironsource/Rf;-><init>(Lcom/ironsource/Lf;Lcom/ironsource/S8;Lcom/ironsource/Sf;)V

    .line 24
    const-string p1, "TestSuiteBridge"

    invoke-static {v1, v2, p1}, Lcom/safedk/android/internal/partials/IronSourceNetworkBridge;->webviewAddJavascriptInterface(Landroid/webkit/WebView;Ljava/lang/Object;Ljava/lang/String;)V

    .line 28
    iget-object p1, p0, Lcom/ironsource/cg;->c:Landroid/webkit/WebView;

    .line 29
    new-instance v0, Lcom/ironsource/cg$a;

    invoke-direct {v0, p0, p3}, Lcom/ironsource/cg$a;-><init>(Lcom/ironsource/cg;Ljava/lang/String;)V

    .line 30
    invoke-virtual {p1, v0}, Landroid/webkit/WebView;->setWebViewClient(Landroid/webkit/WebViewClient;)V

    if-eqz p3, :cond_3

    .line 54
    invoke-interface {p3}, Ljava/lang/CharSequence;->length()I

    move-result p1

    if-nez p1, :cond_2

    goto :goto_2

    .line 55
    :cond_2
    iget-object p1, p0, Lcom/ironsource/cg;->c:Landroid/webkit/WebView;

    invoke-static {p1, p3}, Lcom/safedk/android/internal/partials/IronSourceNetworkBridge;->webviewLoadUrl(Landroid/webkit/WebView;Ljava/lang/String;)V

    .line 57
    :cond_3
    :goto_2
    iget-object p1, p0, Lcom/ironsource/cg;->b:Landroid/webkit/WebView;

    if-eqz p1, :cond_4

    const-string p3, "javascript:document.write(atob(\'\'))"

    invoke-static {p1, p3}, Lcom/safedk/android/internal/partials/IronSourceNetworkBridge;->webviewLoadUrl(Landroid/webkit/WebView;Ljava/lang/String;)V

    .line 58
    :cond_4
    iget-object p1, p0, Lcom/ironsource/cg;->a:Landroid/os/Handler;

    new-instance p3, Lcom/ironsource/cg$$ExternalSyntheticLambda1;

    invoke-direct {p3, p0}, Lcom/ironsource/cg$$ExternalSyntheticLambda1;-><init>(Lcom/ironsource/cg;)V

    const-wide/16 v0, 0x7d0

    invoke-virtual {p1, p3, v0, v1}, Landroid/os/Handler;->postDelayed(Ljava/lang/Runnable;J)Z

    .line 61
    iget-object p1, p0, Lcom/ironsource/cg;->a:Landroid/os/Handler;

    new-instance p3, Lcom/ironsource/cg$$ExternalSyntheticLambda2;

    invoke-direct {p3, p2}, Lcom/ironsource/cg$$ExternalSyntheticLambda2;-><init>(Lcom/ironsource/S8;)V

    const-wide/16 v0, 0x1388

    invoke-virtual {p1, p3, v0, v1}, Landroid/os/Handler;->postDelayed(Ljava/lang/Runnable;J)Z

    return-void
.end method

.method public static final synthetic b(Lcom/ironsource/cg;)Z
    .locals 0

    .line 1
    iget-boolean p0, p0, Lcom/ironsource/cg;->e:Z

    return p0
.end method

.method public static final synthetic c(Lcom/ironsource/cg;)Lorg/json/JSONObject;
    .locals 0

    .line 1
    iget-object p0, p0, Lcom/ironsource/cg;->d:Lorg/json/JSONObject;

    return-object p0
.end method

.method public static final synthetic d(Lcom/ironsource/cg;)Z
    .locals 0

    .line 1
    iget-boolean p0, p0, Lcom/ironsource/cg;->f:Z

    return p0
.end method

.method private static final e(Lcom/ironsource/cg;)V
    .locals 1

    const-string v0, "this$0"

    invoke-static {p0, v0}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullParameter(Ljava/lang/Object;Ljava/lang/String;)V

    const/4 v0, 0x1

    .line 1
    iput-boolean v0, p0, Lcom/ironsource/cg;->f:Z

    return-void
.end method


# virtual methods
.method public final a(Ljava/lang/String;Ljava/util/List;)Ljava/lang/String;
    .locals 2
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "Ljava/lang/String;",
            "Ljava/util/List<",
            "+",
            "Ljava/lang/Object;",
            ">;)",
            "Ljava/lang/String;"
        }
    .end annotation

    const-string v0, "methodName"

    invoke-static {p1, v0}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullParameter(Ljava/lang/Object;Ljava/lang/String;)V

    const-string v0, "methodArgs"

    invoke-static {p2, v0}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullParameter(Ljava/lang/Object;Ljava/lang/String;)V

    .line 72
    new-instance v0, Ljava/lang/StringBuilder;

    const-string v1, "TestSuiteJs."

    invoke-direct {v0, v1}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    invoke-virtual {v0, p1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object p1

    invoke-virtual {p1}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object p1

    .line 73
    sget-object v0, Lcom/ironsource/bg;->a:Lcom/ironsource/bg;

    invoke-virtual {v0, p2}, Lcom/ironsource/bg;->b(Ljava/util/List;)Ljava/lang/String;

    move-result-object p2

    .line 76
    invoke-interface {p2}, Ljava/lang/CharSequence;->length()I

    move-result v0

    if-lez v0, :cond_0

    .line 77
    new-instance v0, Ljava/lang/StringBuilder;

    invoke-direct {v0}, Ljava/lang/StringBuilder;-><init>()V

    invoke-virtual {v0, p1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object p1

    const-string v0, "("

    invoke-virtual {p1, v0}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object p1

    invoke-virtual {p1, p2}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object p1

    const-string p2, ")"

    invoke-virtual {p1, p2}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object p1

    invoke-virtual {p1}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object p1

    return-object p1

    .line 79
    :cond_0
    new-instance p2, Ljava/lang/StringBuilder;

    invoke-direct {p2}, Ljava/lang/StringBuilder;-><init>()V

    invoke-virtual {p2, p1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object p1

    const-string p2, "()"

    invoke-virtual {p1, p2}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object p1

    invoke-virtual {p1}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object p1

    return-object p1
.end method

.method public final a()V
    .locals 1

    .line 80
    invoke-virtual {p0}, Lcom/ironsource/cg;->b()V

    .line 81
    iget-object v0, p0, Lcom/ironsource/cg;->c:Landroid/webkit/WebView;

    invoke-virtual {v0}, Landroid/webkit/WebView;->destroy()V

    return-void
.end method

.method public final a(Ljava/lang/String;)V
    .locals 2

    const-string v0, "script"

    invoke-static {p1, v0}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullParameter(Ljava/lang/Object;Ljava/lang/String;)V

    .line 70
    iget-object v0, p0, Lcom/ironsource/cg;->a:Landroid/os/Handler;

    new-instance v1, Lcom/ironsource/cg$$ExternalSyntheticLambda0;

    invoke-direct {v1, p0, p1}, Lcom/ironsource/cg$$ExternalSyntheticLambda0;-><init>(Lcom/ironsource/cg;Ljava/lang/String;)V

    invoke-virtual {v0, v1}, Landroid/os/Handler;->post(Ljava/lang/Runnable;)Z

    return-void
.end method

.method public a(Ljava/lang/String;Lcom/unity3d/mediation/LevelPlay$AdFormat;Ljava/util/List;)V
    .locals 1
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "Ljava/lang/String;",
            "Lcom/unity3d/mediation/LevelPlay$AdFormat;",
            "Ljava/util/List<",
            "+",
            "Ljava/lang/Object;",
            ">;)V"
        }
    .end annotation

    const-string v0, "callbackName"

    invoke-static {p1, v0}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullParameter(Ljava/lang/Object;Ljava/lang/String;)V

    const-string v0, "adFormat"

    invoke-static {p2, v0}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullParameter(Ljava/lang/Object;Ljava/lang/String;)V

    const-string v0, "args"

    invoke-static {p3, v0}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullParameter(Ljava/lang/Object;Ljava/lang/String;)V

    .line 63
    iget-boolean v0, p0, Lcom/ironsource/cg;->e:Z

    if-eqz v0, :cond_0

    .line 67
    invoke-virtual {p2}, Lcom/unity3d/mediation/LevelPlay$AdFormat;->getValue()Ljava/lang/String;

    move-result-object p2

    filled-new-array {p1, p2, p3}, [Ljava/lang/Object;

    move-result-object p1

    invoke-static {p1}, Lkotlin/collections/CollectionsKt;->listOf([Ljava/lang/Object;)Ljava/util/List;

    move-result-object p1

    .line 68
    const-string p2, "onCallbackReceived"

    invoke-virtual {p0, p2, p1}, Lcom/ironsource/cg;->a(Ljava/lang/String;Ljava/util/List;)Ljava/lang/String;

    move-result-object p1

    .line 69
    invoke-virtual {p0, p1}, Lcom/ironsource/cg;->a(Ljava/lang/String;)V

    :cond_0
    return-void
.end method

.method public final b()V
    .locals 1

    .line 2
    iget-object v0, p0, Lcom/ironsource/cg;->b:Landroid/webkit/WebView;

    if-eqz v0, :cond_1

    if-eqz v0, :cond_0

    .line 3
    invoke-virtual {v0}, Landroid/webkit/WebView;->destroy()V

    :cond_0
    const/4 v0, 0x0

    .line 4
    iput-object v0, p0, Lcom/ironsource/cg;->b:Landroid/webkit/WebView;

    :cond_1
    return-void
.end method

.method public final c()Landroid/webkit/WebView;
    .locals 1

    .line 2
    iget-object v0, p0, Lcom/ironsource/cg;->c:Landroid/webkit/WebView;

    return-object v0
.end method

.method public final d()Landroid/webkit/WebView;
    .locals 1

    .line 2
    iget-object v0, p0, Lcom/ironsource/cg;->b:Landroid/webkit/WebView;

    return-object v0
.end method
